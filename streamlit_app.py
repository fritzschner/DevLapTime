...
def main():
    ...
    with st.form("eingabe_formular"):
        ...
        with col2:
            raw_input = st.text_input("Rundenzeit (6 Ziffern, Format: MSSTTT)", key="zeit_input")

            # Live Formatierung
            formatted = ""
            if raw_input.isdigit():
                if len(raw_input) >= 1:
                    formatted = raw_input[0] + ":"
                if len(raw_input) >= 3:
                    formatted += raw_input[1:3] + "."
                    if len(raw_input) > 3:
                        formatted += raw_input[3:6]
            if formatted:
